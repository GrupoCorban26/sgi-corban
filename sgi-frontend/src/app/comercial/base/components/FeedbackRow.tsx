...
import { Loader2, CheckCircle2, AlertCircle, UserPlus, Pencil } from 'lucide-react';
...
export default function FeedbackRow({
  ...
  const fb = feedback;
  const isComplete = !!(fb.caso_id && fb.comentario);
  const isSaved = contacto.completado && !isEdited;

  // Modo edición local: false = bloqueado (read-only tras guardar), true = editando
  const [editing, setEditing] = useState(!contacto.completado);

  // Determinar estilo de la fila
  const rowStyle = isSaved && !editing
    ? ESTADO_STYLES.completo
    : isComplete
      ? ESTADO_STYLES.editando
      : ESTADO_STYLES.incompleto;

  // Campos bloqueados cuando está guardado y no en modo edición
  const isLocked = isSaved && !editing;
  ...
  const handleGuardar = () => {
    onGuardar(contacto);
    // Después de guardar exitoso, se bloquea (el refetch actualizará contacto.completado)
    setEditing(false);
  };
  ...
  return (
    <tr className={`${rowStyle} transition-all duration-200 hover:shadow-sm`}>
      ...
      <td className="px-3 py-2">
        {isLocked ? (
          <span className={`inline-flex items-center px-3 py-1.5 text-xs font-medium rounded-xl ${
            fb.contesto === true
            ...
            {fb.contesto === true ? '✓ Sí' : fb.contesto === false ? '✗ No' : '—'}
          </span>
        ) : (
          <select
      ...
      <td className="px-3 py-2">
        {isLocked ? (
          <span className="text-xs text-gray-700 block max-w-[200px] truncate" title={fb.comentario}>
            {fb.comentario || '—'}
          </span>
        ) : (
          <input
      ...
      {/* Estado + Guardar/Editar */}
      <td className="px-3 py-2">
        <div className="flex items-center gap-2 justify-center">
          {isLocked ? (
            <button
              onClick={handleEditar}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-amber-400 to-orange-400 text-white text-xs font-semibold rounded-lg shadow-sm hover:shadow-md hover:from-amber-500 hover:to-orange-500 transition-all duration-200"
            >
              <Pencil size={12} />
              Editar
            </button>
