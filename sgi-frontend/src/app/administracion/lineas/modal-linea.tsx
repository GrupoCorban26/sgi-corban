...
export default function ModalLinea({ isOpen, onClose, linea }: ModalLineaProps) {
    ...
    const onSubmit = async (data: LineaCreate) => {
        try {
            ...
            toast.error(err?.response?.data?.detail || 'Error al guardar la línea');
        }
    };
    ...
    return (
        <ModalBase isOpen={isOpen} onClose={onClose} maxWidth="max-w-lg">
            <ModalHeader
                ...
                title={isEditing ? 'Editar Línea' : 'Nueva Línea Corporativa'}
            />

            <form onSubmit={handleSubmit(onSubmit)} className="p-5 space-y-4 overflow-y-auto flex-1 max-h-[60vh]">
                ...
                <ModalFooter>
                    ...
                    <button
                        type="submit"
                        disabled={isLoading}
                        className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-colors disabled:opacity-50 text-sm font-semibold shadow-lg shadow-indigo-200"
                    >
