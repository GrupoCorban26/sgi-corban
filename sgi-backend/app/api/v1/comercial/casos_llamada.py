...
from app.services.casos_llamada_service import CasosLlamadaService
from app.schemas.casos_llamada import CasoLlamadaResponse, CasoLlamadaCreate, CasoLlamadaUpdate
...
async def get_all_casos(db: AsyncSession = Depends(get_db)):
...
async def get_caso(id: int, db: AsyncSession = Depends(get_db)):
...
async def delete_caso(id: int, db: AsyncSession = Depends(get_db)):
